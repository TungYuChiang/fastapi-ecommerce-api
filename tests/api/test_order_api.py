...
@pytest.mark.api
@pytest.mark.asyncio
async def test_get_user_orders(client: AsyncClient, order_data):
    """
    ...
    # Create orders with authentication
    headers = {"Authorization": f"Bearer {token}"}
    await client.post("/orders/", json=order_data, headers=headers)
    await client.post("/orders/", json=order_data, headers=headers)
    
    # Get user's orders
    response = await client.get("/orders/my-orders", headers=headers)
    assert response.status_code == 200
    orders = response.json()
    assert isinstance(orders, list)
    assert len(orders) >= 2  # At least the two we just created


@pytest.mark.api
@pytest.mark.asyncio
async def test_cancel_order(client: AsyncClient, order_data):
    """
